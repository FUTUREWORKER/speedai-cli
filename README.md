# Wooboo AI / Speed AI CLI

`speedai` 是挖宝AI（Wooboo AI）的本地命令行入口。它使用用户网页登录授权后的系统 token 调用平台 API，不直连模型供应商；积分扣除、任务队列、OSS 存储和创作历史均由平台统一处理。

命令名、包名、配置目录和生产域名继续保留 `speedai`，以兼容已有安装和本机凭据。CLI 只有一个版本，通过配置切换生产或本地开发环境。
...
CLI 负责执行命令，Agent Skill 负责告诉 Codex、OpenClaw、Hermes 等 Agent 何时以及如何调用 `speedai`。

推荐按需安装：

```bash
npx -y skills add FUTUREWORKER/speedai-cli --skill speedai-image-generation
...
npx -y skills add FUTUREWORKER/speedai-cli --skill speedai-video-kling-3-0
npx -y skills add FUTUREWORKER/speedai-cli --skill speedai-long-video-creation
npx -y skills add FUTUREWORKER/speedai-cli --skill speedai-digital-human-avatar
...
默认连接生产环境 `https://speed.ycszai.com`。用户需要在打开的 H5 页面中登录并确认授权。
...
```bash
speedai image generate --prompt "高级棚拍产品图，黑色无线音箱放在混凝土桌面上"
```
...
```bash
speedai avatar create --video /path/to/avatar-training.mp4 --title "品牌主理人"
```
...
旧 `speedai audio synthesize` 独立音频接口已经下线。CLI 暂时保留该命令用于返回明确迁移提示，不再向已删除的接口提交请求。
...
```bash
speedai config set output-dir /path/to/generated-media
...
1. `--output-dir`
2. `SPEEDAI_OUTPUT_DIR`
3. `speedai config set output-dir ...`
...
用户自然语言
  -> Agent 触发对应通用 Skill
  -> Agent 执行 speedai image/video/avatar/voice/digital-human ...
